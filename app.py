...
def init_db():
    ...
    CREATE INDEX IF NOT EXISTS idx_deposit_reference
    ON deposits(reference);

    CREATE INDEX IF NOT EXISTS idx_commission_source
    ON referral_commissions(source_user_id);

    CREATE INDEX IF NOT EXISTS idx_commission_receiver
    ON referral_commissions(user_id);
    """)
...
def distribute_referral_commission(conn, source_user_id, base_amount):
    """
    Pays:
        Level 1 = 25%
        Level 2 = 3%
    ...
    The commission is paid only once for each
    source deposit and each referral level.
    """

    current_id = source_user_id
    ...
    for level in range(1, 4):
        ...
        if not already_paid and commission > 0:
...
def register():
    ...
    if request.method == "POST":
        ...
        flash(
            "NEXA account created successfully.",
            "success"
        )
...
def withdraw():
    ...
    if request.method == "POST":
        ...
        if not bank:
            ...
            flash(
                "Please bind your bank account first.",
                "error"
            )
        ...
        flash(
            f"Withdrawal submitted. You will receive "
            f"{receive_amount:.2f} ETB after the 10% fee.",
            "success"
        )
...
def admin_dashboard():
    ...
    recent_commissions = conn.execute(
        """
        SELECT
        ...
        FROM referral_commissions
        JOIN users receiver
        ON receiver.id = referral_commissions.user_id
        JOIN users source
        ON source.id = referral_commissions.source_user_id
        ORDER BY referral_commissions.id DESC
        LIMIT 100
        """
    ).fetchall()
    ...
    stats = {
        ...
        "total_commissions": conn.execute(
            """
            SELECT COALESCE(SUM(amount),0) s
            FROM referral_commissions
            """
...
def approve_deposit(deposit_id):
    ...
    # AUTOMATIC COMMISSION ON APPROVE
    # 25% / 3% / 2%
    # Duplicate protection is inside the function.
    ...
    flash(
        "Deposit approved, balance credited and referral commissions processed.",
        "success"
    )
...
@app.route(
    "/admin/withdraw/<int:withdrawal_id>/paid",
    methods=["POST"]
)
@admin_required
def mark_withdrawal_paid(withdrawal_id):
    ...
    if (
        not withdrawal
        or withdrawal["status"] != "PROCESSING"
    ):
...
def reject_withdrawal(withdrawal_id):
    ...
    flash(
        "Withdrawal rejected and reserved balance returned.",
        "success"
    )
...
@app.route(
    "/admin/plan/<int:deposit_id>/activate",
    methods=["POST"]
)
@admin_required
def activate_plan(deposit_id):
    ...
    # Commission is NOT paid here.
    # It is already paid at APPROVE.
