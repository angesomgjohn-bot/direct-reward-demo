...
CUSTOMER_SERVICE_TEXT = os.environ.get("CUSTOMER_SERVICE_TEXT", "NEXA Customer Service — @NexaSupport11")
...
def now():
    return datetime.now(ETHIOPIA_TZ).isoformat(timespec="seconds")
...
def render_home(user=None, referral_link=None):
    ...
    return render_template(
        ...
        customer_service_text=CUSTOMER_SERVICE_TEXT,
...
def deposit():
    ...
    try:
        existing = conn.execute(
            "SELECT 1 FROM deposits WHERE reference=? LIMIT 1", (reference,)
        ).fetchone()
        if existing:
...
def withdraw():
    ...
    hour = datetime.now(ETHIOPIA_TZ).hour
